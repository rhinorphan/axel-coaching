...
function Contact() {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm('service_uclnfjc', 'template_zlbjyo2', form.current, '4QWpwWp_TI28hRNyX')
      .then((result) => {
  ...
  <div class="py-8 lg:py-16 px-4 mx-auto max-w-screen-md dark:glassmorphism backdrop-blur-lg backdrop-opacity-100 backdrop-contrast-75 shadow-md">
      <h2 class="mb-4 text-4xl tracking-tight font-extrabold text-center text-gray-900 dark:text-gray-200">Me contacter</h2>
      <p class="mb-8 lg:mb-16 font-light text-center dark:text-gray-300 text-gray-800 sm:text-xl">Lorem ipsum dolor sit amet, consectetur adipisicing elit? A soluta corporis voluptate ab error quam dolores doloremque. Quae consectetur.</p>
      <form action="#" class="space-y-8" ref={form} onSubmit={sendEmail}>
          ...
          <button type="submit" class="py-3 px-5 text-sm font-medium text-center dark:text-gray-100text-gray-800 rounded-lg bg-primary-700 sm:w-fit hover:bg-primary-800 focus:ring-4 focus:outline-none focus:ring-primary-300 dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">Envoyer</button>
      </form>
  </div>
</section>
  )
